...
# from losses.loss import euclidean_distance_loss, binary_cross_entropy_loss
...
def train_one_epoch(model: torch.nn.Module, criterion: callable,
    ...
    for samples, targets in data_loader:
        # import json
        # print(json.dumps(samples.tolist()))
        # exit(0)
        samples = samples.to(device)
        targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
        # print(targets)
        gt_density = targets[0]['point']
        # forward
        outputs = model(samples)
        # calc the losses
        # loss_dict = criterion(outputs, targets)
        # weight_dict = criterion.weight_dict
        # loss1 = euclidean_distance_loss(outputs, targets)
        # loss2 = binary_cross_entropy_loss(outputs, targets)
        # # combine the losses
        # loss = loss1 + loss2
